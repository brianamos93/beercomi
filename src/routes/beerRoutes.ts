...
async function beerlookup(beerID: String) {
	return await pool.query("SELECT id, name, brewery_id, description, ibu, abv, color, author_id, style, cover_image FROM beers WHERE id = $1", [beerID]);
  }
...
async function beerUser(beerID: String) {
 ...
 async function userBeerLookup(authorID: String, beerID: String) {
	return await pool.query("SELECT id FROM beer_reviews WHERE author_id = $1 AND beer_id = $2", [authorID, beerID])
 }
...
async function photoLookup(photoId: String) {
	return await pool.query("SELECT user_id, photo_url FROM review_photos WHERE id = $1", [photoId])
}
...
//create new review
 router.post("/review/", authenticationHandler, upload.array('photos', 4), async (req: Request, res: Response) => {
	...
	if (reviewCheck.rowCount != 0) {
		return res.status(409).json({error: 'Review already exists for this user.'})
	}
	...
	try {
		...
		if (files && files.length > 0) {
			const photoInserts = files.map(async (file, index) => {
				const uploadPath = path.join(__dirname, '..', `uploads/${breweryName}/${beerName}`);
				if (!fs.existsSync(uploadPath)) {
					fs.mkdirSync(uploadPath, { recursive: true });
				}
...
//update review
router.put("/review/:id", authenticationHandler, upload.array('photos', 4), async (req: Request, res: Response) => {
	const reviewID = req.params.id
	const client = await pool.connect();
	const { rating, review, beer_id } = req.body;
	...
	try {
		...
		client.query("UPDATE beer_reviews SET rating = $1, review = $2, beer_id = $3 WHERE id = $4", [
		numberRating,
		review,
		trimmedBeer_id,
		reviewID
	...
	if (files && files.length > 0) {
		...
		if (reviewPhotoNumber != null && (reviewPhotoNumber === 4 || requestPhotosCount + reviewPhotoNumber === 4)) {
...
//delete review photo
router.delete("/review/photo/:id", authenticationHandler, express.json(), async(req: Request, res: Response) => {
	...
	if(photoCheck.rows[0].user_id !== req.user!.id && req.user!.role != 'admin') {
		return res.status(401).json({error: 'Unauthorized'})
	}
	try {
...
//delete review
router.delete("/review/:id", authenticationHandler, express.json(), async (req: Request, res: Response) => {
	const reviewID = req.params.id
	const reviewcheck = await reviewLookup(reviewID)
	if (reviewcheck.rowCount == 0) {
